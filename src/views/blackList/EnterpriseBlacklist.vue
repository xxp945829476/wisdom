...
import {VehicleDeptList,BaseList,RemoveDeptBlack,ExportDeptBlack} from '@/network/api'
...
export default {
  ...
  methods:{
    ...
    getData(){
         this.loading = true;
        VehicleDeptList(this.formParmas).then(res=>{
             this.loading = false;
            if(res.data.code == 0){
    ...
    addQuan(){
      this.$refs.add_quan.addQuan(1)
    },
   
    viewDetails(record){
      this.$refs.view_details.viewDetails(record,1)
    },
    ...
    removeBlackOK(){
      let parmas = {
        ...
        RemoveDeptBlack(parmas).then(res=>{
            if(res.data.code == 0){
                this.getData()
    ...
    exportBlack(){
      ...
      ExportDeptBlack(params).then(res=>{
          if(window.navigator.msSaveBlob){
            const blobObject = new Blob([res.data]);
            window.navigator.msSaveBlob(blobObject, '企业黑名单.xls');
          }else{
            let blob = new Blob([res.data]); // 假设文件为pdf
            let link = document.createElement('a');
            link.href = window.URL.createObjectURL(blob);
            link.download = '企业黑名单.xls';
            link.click();
            link.remove();
          }
