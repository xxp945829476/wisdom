...
<script>
import {BaseList,AddSiteEquipment,EditSiteEquipment,DepartmentList} from '@/network/api'
...
export default {
  ...
  methods:{
    ...
    onSubmit(){
      this.spinning = true;
      AddSiteEquipment(this.addForm).then(res=>{
          this.spinning = false;
          if(res.data.code == 0){
     ...
     onEditSubmit(){
       this.spinning = true;
      EditSiteEquipment(this.addForm).then(res=>{
          this.spinning = false;
          if(res.data.code == 0){
            this.equipmentVisible = false;
            this.$message.success('保存成功');
