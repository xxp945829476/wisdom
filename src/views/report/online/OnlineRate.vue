    ...
    <a-row type="flex">
      ...
      <a-col flex="auto">
        ...
        <a-table :columns="columns" bordered :data-source="tableData" @change="changeTable" :rowKey='record' size="middle" :pagination="pagination" :loading="loading" :scroll="{y:height}">
              <span slot="onlineRate" slot-scope="text">
                  <span>{{text}}%</span>
              </span>
        </a-table>
...
<script>
import {IndexTree,onlineOfflineSummary,ExportOnlineOfflineSummary} from "@/network/api"
export default {
  data() {
    const columns = [
      ...
      },
      {
        ...
        align:'center',
        ellipsis:true,
        scopedSlots: { customRender: 'onlineRate' },
      }
    ];
    return {
      height:'',
  ...
  methods:{
    ...
    getDay(){
      ...
      this.formParmas.begintime = this.$moment(new Date()).subtract(1,'months').format('YYYY-MM-DD') + ' ' + '00:00:00'
    ...
    getData(){
        this.loading = true;
        onlineOfflineSummary(this.formParmas).then(res=>{
            this.loading = false;
            if(res.data.code == 0){
    ...
    exportReport(){
      ...
      ExportOnlineOfflineSummary(params).then(res=>{
          if(window.navigator.msSaveBlob){
            const blobObject = new Blob([res.data]);
            window.navigator.msSaveBlob(blobObject, '上线率.xls');
          }else{
            let blob = new Blob([res.data]); // 假设文件为pdf
            let link = document.createElement('a');
            link.href = window.URL.createObjectURL(blob);
            link.download = '上线率.xls';
            link.click();
            link.remove();
          }
