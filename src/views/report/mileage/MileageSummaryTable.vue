                      ...
                      <a-row :gutter="24">
                          ...
                          <a-col :md="12">
                              <a-form-model-item label="开始时间">
                                 <a-date-picker show-time placeholder="请选择开始时间" v-model="formParmas.begintime" :disabled-date="disabledStartDate" format="YYYY-MM-DD" :allowClear="false"  valueFormat="YYYY-MM-DD"/>
                              </a-form-model-item>
                          </a-col>
                          <a-col :md="12">
                              <a-form-model-item label="结束时间">
                                 <a-date-picker show-time  placeholder="请选择结束时间" v-model="formParmas.endtime" :disabled-date="disabledEndDate" format="YYYY-MM-DD" :allowClear="false" valueFormat="YYYY-MM-DD"/>
                              </a-form-model-item>
                          </a-col>
...
<script>
import {IndexTree,MileageSummary,ExportSummary} from "@/network/api"
...
export default {
  ...
  methods:{
    ...
    getDay(){
      ...
      this.formParmas.begintime = this.$moment(new Date()).subtract(1,'months').format('YYYY-MM-DD');
      this.formParmas.endtime = day;
    },
    getVehicleList(){
      this.spinning = true;
    ...
    getData(){
        this.loading = true;
        MileageSummary(this.formParmas).then(res=>{
            this.loading = false;
            if(res.data.code == 0){
    ...
    exportReport(){
      ...
      ExportSummary(params).then(res=>{
          if(window.navigator.msSaveBlob){
            const blobObject = new Blob([res.data]);
            window.navigator.msSaveBlob(blobObject, '里程汇总报表.xls');
          }else{
            let blob = new Blob([res.data]); // 假设文件为pdf
            let link = document.createElement('a');
            link.href = window.URL.createObjectURL(blob);
            link.download = '里程汇总报表.xls';
            link.click();
            link.remove();
          }
