...
<script>
import {IndexTree,DrivingAlarm,ExportAlarm} from "@/network/api"
...
export default {
  ...
  methods:{
    ...
    getData(){
        this.loading = true;
        DrivingAlarm(this.formParmas).then(res=>{
            this.loading = false;
            if(res.data.code == 0){
    ...
    exportReport(){
      ...
      ExportAlarm(params).then(res=>{
          if(window.navigator.msSaveBlob){
            ...
            window.navigator.msSaveBlob(blobObject, 'ACC点火查询报表.xls');
          }else{
            let blob = new Blob([res.data]); // 假设文件为pdf
            let link = document.createElement('a');
            link.href = window.URL.createObjectURL(blob);
            link.download = 'ACC点火查询报表.xls';
            link.click();
            link.remove();
          }
